    ...
    <div className="w-full max-w-md mx-auto space-y-6">
      <Card className="public-card overflow-hidden">
        <CardContent className="p-0">
          <div className="bg-foreground text-background aspect-[4/3] relative flex flex-col items-center justify-center overflow-hidden">
            ...
            {isScanning && !hasScanned && (
              <div className="absolute inset-0 z-10 pointer-events-none">
                <div className="w-full h-full border-[40px] border-background/30">
                  <div className="w-full h-full border-2 border-background/60 relative">
                    <div className="absolute top-0 left-0 w-4 h-4 border-t-4 border-l-4 border-brand-teal -mt-1 -ml-1"></div>
                    <div className="absolute top-0 right-0 w-4 h-4 border-t-4 border-r-4 border-brand-teal -mt-1 -mr-1"></div>
                    <div className="absolute bottom-0 left-0 w-4 h-4 border-b-4 border-l-4 border-brand-teal -mb-1 -ml-1"></div>
                    <div className="absolute bottom-0 right-0 w-4 h-4 border-b-4 border-r-4 border-brand-teal -mb-1 -mr-1"></div>
                  </div>
                </div>
              </div>
            )}
            ...
            {hasScanned && scanResult && (
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-brand-teal/90 text-foreground gap-3 px-4 text-center">
                <CheckCircle2 className="h-12 w-12" />
                <p className="font-semibold">{scanResult}</p>
                <p className="text-xs text-foreground/70">Redirecting to delivery form...</p>
              </div>
            )}

            {/* Idle / Error / Requesting States */}
            {!isScanning && !hasScanned && (
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center p-6 text-center bg-foreground">
                {isRequestingCamera ? (
                  <>
                  ...
                  <>
                    <VideoOff className="h-8 w-8 text-destructive mb-3" />
                    <p className="text-sm font-medium mb-1">Camera Permission Denied</p>
                    <p className="text-xs text-background/70 max-w-[250px]">
                  ...
                  <>
                    ...
                    <p className="text-xs text-background/70 max-w-[250px]">{scanError}</p>
                  </>
                ) : (
                  <>
                    <QrCode className="h-10 w-10 text-background/50 mb-3" />
            ...
            {scanError && !isScanning && !hasScanned && !permissionDenied && (
              <div className="absolute bottom-4 left-0 right-0 flex justify-center z-30">
                <Button size="sm" variant="outline" className="border-background/30 bg-background/10 text-background hover:bg-background/20 hover:text-background" onClick={() => startCamera()}>
                  Retry Camera
                </Button>
              </div>
            )}
          ...
          <div className="p-4 bg-card border-t border-border/60 space-y-4">
            ...
            {scanError && !hasScanned && (
              <div className="p-3 text-sm text-destructive bg-destructive/10 border border-destructive/25 rounded-2xl flex gap-2 items-start" role="alert">
          ...
          <form onSubmit={handleManualSubmit} className="space-y-3">
            <div className="space-y-1">
              ...
              <Input
                ...
                className={manualError ? "border-destructive focus-visible:ring-destructive/40" : ""}
              />
              {manualError && (
                <p className="text-xs text-destructive font-medium" role="alert">{manualError}</p>
              )}
            </div>
